...
# ## DROP outliers, repeat the modeling / smoothing process
# dat2 <- dat
# dat2[, is.na(df_sample_pred$out_marss) | df_sample_pred$out_marss != '0'] <- NA
# 
#     
# kem2 <- MARSS(dat2, model = kemod, method = "kem")
#   
# df_sample_pred2 <- df_sample_pred %>%
#   mutate( Longitude.Pred = kem2$states[1,], 
#           Latitude.Pred = kem2$states[2,]
#   )
# 
# 
#   
#   ## 2d trajectory
#   p_latlon <-  df_sample_pred2 %>% 
#     ggplot(  ) +
#     geom_point( aes(x = Longitude, y= Latitude, color = out_marss) )+
#     scale_discrete_manual("color", values = col_out_scale)+
#     geom_line(aes(x = Longitude.Pred, y= Latitude.Pred), color = "#333333")+
#     # xlim(-112.46, -112.47)+ylim(34.7,34.715)+
#     theme_minimal()+
#     coord_fixed()
#   
#   ##  time series for lat / lon
#   p_lat <- df_sample_pred2 %>% 
#     ggplot( aes(x = TimeElapsed) ) +
#     geom_point( aes(y= Latitude, color = out_marss) )+
#     scale_discrete_manual("color", values = col_out_scale)+
#     geom_line(aes(y= Latitude.Pred), color = "#333333")+
#     # xlim(2010000, 2070000)+
#     theme_minimal()
#   
#   p_lon <-  df_sample_pred2 %>% 
#     ggplot( aes(x = TimeElapsed) ) +
#     geom_point( aes(y= Longitude, color = out_marss) )+
#     scale_discrete_manual("color", values = col_out_scale)+
#     geom_line(aes(y= Longitude.Pred), color = "#333333")+
#     # xlim(2010000, 2070000)+
#     theme_minimal()
...
my_clusters <- cluster_analyze(df, "2018-06-25", knn_eps = cow_eps) # WORKS
my_clusters <- cluster_analyze(df, "2018-06-26", knn_eps = car_eps) # ??


my_clusters <- cluster_analyze(df, "2018-06-16", knn_eps = cow_eps) # too detailed?

my_clusters <- cluster_analysis_by_date(df, "2018-05-24", knn_eps = car_eps ) # doesn't work?

my_clusters <- cluster_analysis_by_date(df, "2018-05-23", knn_eps = car_eps ) # doesn't work?
...
# filter(Date %in% c("2018-06-16", "2018-06-17", "2018-06-18", "2018-06-19") ) %>%
filter(Date == ) %>%
